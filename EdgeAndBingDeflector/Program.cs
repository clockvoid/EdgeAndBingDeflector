...
namespace EdgeDeflector
{
    class Program
    {
		...
		static bool IsSearch(string uri)
		{
			string search_pattern = ".*search?.*q=.*";
			Regex match_search = new Regex(search_pattern);
			return match_search.IsMatch(uri);
		}

		static String CreateSearchUri(string uri)
		{
			String new_uri = "";
		...
		static string RewriteMsEdgeUriSchema(string uri)
        {
			...
			if (IsSearch(new_uri))
			{
				new_uri = CreateSearchUri(new_uri);
			}
